...
Deno.serve(async(request:Request)=>{
  ...
  try{
    ...
    const {data:target,error:targetError}=await admin.from('profiles').select('id,role,is_active').eq('id',body.userId).single();
    if(targetError||!target)return json({error:'Utilizador não encontrado.'},404);
    if(target.role==='platform_owner')return json({error:'A conta do proprietário da plataforma está protegida.'},403);
