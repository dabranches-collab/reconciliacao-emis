create table if not exists private.rt_v2_primary_candidates(
  import_id uuid not null references public.rt_v2_imports(id) on delete cascade,
  reconciliation_key text not null,
  bucket integer not null,
  primary key(import_id,reconciliation_key)
);
create index if not exists rt_v2_primary_candidates_bucket_idx
  on private.rt_v2_primary_candidates(import_id,bucket,reconciliation_key);

create table if not exists private.rt_v2_secondary_candidates(
  import_id uuid not null references public.rt_v2_imports(id) on delete cascade,
  operation_number text not null,
  description_normalized text not null,
  absolute_amount numeric(20,2) not null,
  bucket integer not null,
  primary key(import_id,operation_number,description_normalized,absolute_amount)
);
create index if not exists rt_v2_secondary_candidates_bucket_idx
  on private.rt_v2_secondary_candidates(import_id,bucket,operation_number,description_normalized,absolute_amount);
...
revoke all on private.rt_v2_primary_candidates,private.rt_v2_secondary_candidates from public,anon,authenticated;
