...
begin
  ...
  with bounds as (
    select min(coalesce(accounting_date,movement_date)) first_day,max(coalesce(accounting_date,movement_date)) last_day
    from public.movements where analysis_id=p_analysis_id
  ), open_groups as (
    select m.idtr,min(coalesce(m.accounting_date,m.movement_date)) first_seen,max(coalesce(m.accounting_date,m.movement_date)) last_seen,
