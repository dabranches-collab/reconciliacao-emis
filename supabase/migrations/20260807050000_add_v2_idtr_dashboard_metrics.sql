-- Expõe no dashboard a proveniência do identificador sem criar um IDTR
-- artificial. “Sem IDTR ( /26)” continua a ser uma classificação auditável.
...
begin
  ...
  totals as(select count(*) movements,
    count(*) filter(where status in('reconciled','manual')) reconciled,
    count(*) filter(where status='open') open,
    count(*) filter(where native_idtr is not null) with_idtr,
    count(*) filter(where native_idtr is null) without_idtr,
    count(*) filter(where status in('reconciled','manual') and reconciliation_method='idtr') reconciled_idtr,
    count(*) filter(where status in('reconciled','manual') and native_idtr is null) reconciled_without_idtr,
    round(sum(amount)::numeric,2) amount,
    count(*) filter(where balance_sequence_valid=false) balance_anomalies,
    min(accounting_date) first_date,max(accounting_date) cutoff
    from public.rt_v2_movements where series_id=v_import.series_id)
