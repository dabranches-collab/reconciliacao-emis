...
export default function HistoryDashboard({ revision }: { revision: number }) {
  ...
  const chartDays=daily.length?daily:days.map(day=>[day.reportDate,{movements:day.totals.movements,automatic:day.totals.automatic,unreconciled:day.totals.unreconciled,missingIdtr:day.totals.missingIdtr,amount:day.totals.amount}] as const);
  return <div className="history-page">
    ...
    <section className="history-types"><div className="section-heading"><div><p className="eyebrow">EVOLUÇÃO POR NATUREZA</p><h2>Última fotografia comparada com a anterior</h2></div></div><div className="history-type-grid">{Object.entries(labels).map(([key,label])=>{const now=latest.movementTypes[key]?.total??0;const before=previous?.movementTypes[key]?.total??now;const delta=before?(now-before)/before*100:0;return <article key={key}><span>{label}</span><strong>{now.toLocaleString('pt-AO')}</strong><small className={Math.abs(delta)>=30?'warning':''}>{delta>=0?'+':''}{delta.toFixed(1)}% vs. anterior</small><div>{days.slice(-8).map(day=><i key={day.id} style={{height:`${Math.max(4,((day.movementTypes[key]?.total??0)/Math.max(1,...days.slice(-8).map(d=>d.movementTypes[key]?.total??0)))*48)}px`}}/>)}</div></article>})}</div></section>
