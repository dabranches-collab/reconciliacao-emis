...
export async function analyzeWorkbookBuffer(fileName: string, buffer: ArrayBuffer, onProgress: (progress: AnalysisProgress) => void): Promise<AnalysisResult> {
  ...
  if (rec) {
    ...
    for (let row = 3; row < totalRows; row++) {
      ...
      if (row % 10000 === 0) onProgress({ percent: 12 + Math.round((row / totalRows) * 23), stage: 'A analisar movimentos já classificados', processed: row, total: totalRows, liveTotals: liveTotals() });
    }
  }
  rec = undefined;
  ...
  for (let row = 21; row < rtRows; row++) {
    ...
    if (movement) rtIdentified++;
    if (movement?.idtr) groups.set(movement.idtr, (groups.get(movement.idtr) ?? 0) + Math.round(movement.amount * 100));
    if (row % 10000 === 0) onProgress({ percent: 40 + Math.round((row / rtRows) * 25), stage: 'A agrupar movimentos por IDTR', processed: row, total: rtRows, liveTotals: { ...liveTotals(), movements: totals.movements + rtIdentified } });
  }
  ...
  for (let row = 21; row < rtRows; row++) {
    ...
    addType(movementTypes, movement.description, movement.status); keep(movement);
    if (row % 10000 === 0) onProgress({ percent: 67 + Math.round((row / rtRows) * 28), stage: 'A validar saldos e preparar resultados', processed: row, total: rtRows, liveTotals: liveTotals() });
  }
