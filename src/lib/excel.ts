...
export interface AnalysisProgress { percent: number; stage: string; processed?: number; total?: number; unit?: 'linhas'|'blocos'; storedRows?: number; attempt?: number; heartbeatAt?: string|null; liveTotals?: { movements: number; automatic: number; unreconciled: number; missingIdtr: number }; liveMovementTypes?: Record<string, { total: number; reconciled: number; unreconciled: number; missingIdtr: number }> }
