...
export interface AnalysisProgress { percent: number; stage: string; processed?: number; total?: number; unit?: 'linhas'|'blocos'; storedRows?: number; serverBlock?:number; serverBlocks?:number; attempt?: number; heartbeatAt?: string|null; liveTotals?: { movements: number; automatic: number; unreconciled: number; missingIdtr: number }; liveV2?: { withNativeIdtr:number; withoutNativeIdtr:number; reference26:number; amountCents:number; duplicates:number; rejected:number; provisionalReconciled:number }; liveMovementTypes?: Record<string, { total: number; reconciled: number; unreconciled: number; missingIdtr: number }> }
