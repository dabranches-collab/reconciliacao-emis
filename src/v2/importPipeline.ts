  ...
  try{
    ...
    for await(const row of rows){
      ...
      try{
        ...
        if(!balance.valid){accountingAnomalies++;if(accountingAnomalySamples.length<20)accountingAnomalySamples.push({sourceRow:rowNumber,message:`O saldo contabilístico não corresponde ao saldo anterior acrescido do valor do movimento (esperado ${(balance.expectedBalanceCents/100).toFixed(2)}, recebido ${(balance.actualBalanceCents/100).toFixed(2)}).`});}
