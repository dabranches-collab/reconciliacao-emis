...
export type ImportProgress={stage:ImportStage;processed:number;inserted:number;duplicates:number;rejected:number;message:string;withNativeIdtr:number;withoutNativeIdtr:number;reference26:number;amountCents:number;provisionalReconciled:number};
...
export async function ingestRows(rows:AsyncIterable<unknown[]>,sink:ImportSink,batchSize=1000){
  ...
  const report=async(stage:ImportStage,message:string)=>sink.progress({stage,processed,inserted,duplicates,rejected,message,withNativeIdtr,withoutNativeIdtr,reference26,amountCents,provisionalReconciled});
  try{
    ...
    for await(const row of rows){
      ...
      try{
        ...
        const persisted={...normalized,sourceRow:rowNumber,fingerprint:'',raw,balanceSequenceValid:balance.valid,expectedBalanceCents:balance.valid?normalized.balanceCents:balance.expectedBalanceCents};persisted.fingerprint=fingerprint(persisted);batch.push(persisted);processed++;amountCents+=normalized.amountCents;if(normalized.nativeIdtr){withNativeIdtr++;const before=idtrBalances.get(normalized.nativeIdtr)??{count:0,amountCents:0};if(before.count>=2&&before.amountCents===0)provisionalReconciled-=before.count;const after={count:before.count+1,amountCents:before.amountCents+normalized.amountCents};idtrBalances.set(normalized.nativeIdtr,after);if(after.count>=2&&after.amountCents===0)provisionalReconciled+=after.count;}else withoutNativeIdtr++;if(normalized.reference26)reference26++;
