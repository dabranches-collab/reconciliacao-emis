...
export type ImportProgress={stage:ImportStage;processed:number;inserted:number;duplicates:number;rejected:number;message:string};
...
export async function ingestRows(rows:AsyncIterable<unknown[]>,sink:ImportSink,batchSize=1000){
  let columns:ReturnType<typeof resolveExtractHeaders>['columns']=null,headerRow=0,processed=0,inserted=0,duplicates=0,rejected=0,accountingAnomalies=0,rowNumber=0;const rejectionSamples:ImportRejection[]=[],accountingAnomalySamples:ImportRejection[]=[];
  const balanceValidator=new BalanceSequenceValidator(),batch:PersistedMovement[]=[];
  const report=async(stage:ImportStage,message:string)=>sink.progress({stage,processed,inserted,duplicates,rejected,message});
  try{
    ...
    for await(const row of rows){
      ...
      try{
        ...
        const persisted={...normalized,sourceRow:rowNumber,fingerprint:'',raw,balanceSequenceValid:balance.valid,expectedBalanceCents:balance.valid?normalized.balanceCents:balance.expectedBalanceCents};persisted.fingerprint=fingerprint(persisted);batch.push(persisted);processed++;
