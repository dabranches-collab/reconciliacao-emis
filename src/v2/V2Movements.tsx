...
import {AlertTriangle,Clock3,Database,LoaderCircle,Repeat2} from 'lucide-react';
...
export default function V2Movements(){
  ...
  const load=useCallback(async(reset:boolean)=>{if(!seriesId)return;reset?setLoading(true):setLoadingMore(true);setError('');try{
    let query=supabase.from('rt_v2_movements').select('id,accounting_date,system_date,system_time,operation_number,raw_description,amount,balance,native_idtr,reference_26,status,reconciliation_method').eq('series_id',seriesId).order('accounting_date',{ascending:false}).order('id',{ascending:false});
    if(filter!=='all')query=query.eq('status',filter);const from=reset?0:rows.length,response=await query.range(from,from+PAGE-1);if(response.error)throw response.error;setRows(previous=>reset?(response.data as Movement[]):[...previous,...(response.data as Movement[])]);
  ...
  useEffect(()=>{if(!seriesId)return;void supabase.from('rt_v2_calculations').select('result').eq('series_id',seriesId).eq('metric','dashboard').maybeSingle().then(({data,error})=>{if(error){setError(error.message);return;}const totals=((data?.result as Record<string,unknown>|null)?.totals??{}) as Record<string,unknown>;setCounts({all:Number(totals.movements??0),open:Number(totals.open??0),reconciled:Number(totals.reconciled??0)});});},[seriesId]);
  useEffect(()=>{setTotal(filter==='all'?counts.all:filter==='open'?counts.open:counts.reconciled);},[counts,filter]);
