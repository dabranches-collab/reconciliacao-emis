...
export default function RealTimeOverview({revision}:{revision:number}){
  ...
  return <section className={`rt-overview ${gaps.length?'has-coverage-gap':''}`}>{gaps.length>0&&<div className="coverage-critical" role="alert"><AlertTriangle size={28}/><div><strong>ATENÇÃO — EXISTEM DATAS SEM EXTRATO CARREGADO</strong><p>{gaps.map(g=>`${date(g.from)} a ${date(g.to)}`).join(' · ')}</p></div><span>CARREGAMENTO EM FALTA</span></div>}<div className="overview-head"><div><p className="eyebrow">VISÃO PERMANENTE</p><h2>Estado da Reconciliação Real Time</h2><p>{history.length?'Indicadores calculados pelas fotografias válidas dos extratos importados.':'Importe o primeiro extrato para iniciar a série histórica.'}</p></div><span className={gaps.length?'coverage-alert':'coverage-ok'}>{gaps.length?<><AlertTriangle size={16}/>{gaps.length} intervalo{gaps.length>1?'s':''} em falta</>:<><CheckCircle2 size={16}/>Cobertura contínua</>}</span></div><div className="overview-metrics">
