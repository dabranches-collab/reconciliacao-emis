...
import { Activity, ArrowLeftRight, BarChart3, BookOpen, CheckCircle2, CircleEllipsis, Clock3, CreditCard, FileSpreadsheet, Grid2X2, History, Landmark, Moon, ReceiptText, RefreshCw, ShieldCheck, Sun, Upload, Users, Wrench } from 'lucide-react';
...
export default function App() {
  ...
  return <div className="app-shell">
    ...
    <main><header className="operational-header"><div><p className="eyebrow">PAINEL OPERACIONAL</p><h1>{pageTitle}</h1><p>{pageDescription}</p></div><div className="header-controls"><button type="button" title={theme==='light'?'Ativar modo escuro':'Ativar modo claro'} aria-label={theme==='light'?'Ativar modo escuro':'Ativar modo claro'} onClick={()=>setTheme(value=>value==='light'?'dark':'light')}>{theme==='light'?<Moon size={18}/>:<Sun size={18}/>}</button><button type="button" title="Atualizar dados deste ecrã" aria-label="Atualizar dados deste ecrã" disabled={refreshing} onClick={()=>void refreshData()}><RefreshCw size={18} className={refreshing?'spinning':''}/></button></div></header>
